...
PCS_URL = input('\nEnter URL for PCS startlist:\n')
#PCS_URL = 'https://www.procyclingstats.com/race/brabantse-pijl/2020/startlist'

race = getstrings('first','/race/','/',PCS_URL,0)
year = getstrings('first','/'+race+'/','/',PCS_URL,0)

level = input('\nEnter level (WT, Pro, 1, 2): ')
#level = 'Pro'
...
for i in range(len(riderurls)):
	...
	check2 = list(findstrings(year+'/result',source))
	if check == '0' or check2 == []:
		flag1 = 1
		places = []
		races = []
		kinds = []
